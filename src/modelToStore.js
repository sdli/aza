...
function createApp(options, model) {
  ...
  const __reducers = addLoading(
    addRoutingModel(modelReducersToStore(model, options.persist))
  );
  ...
  const store = createStore(
    ...
    options ||
      ...
      ? ImmutableCombineReducers(__reducers)
      : combineReducers(__reducers),
    options.initialState
      ? immutable.fromJS(
          Object.assign(options.initialState, getReduxPersis(options.persist))
        )
      : immutable.fromJS(getReduxPersis(options.persist)),
    /**
   ...
   */
  this.run = () => {
    ReactDOM.render(this.__domProvider, document.getElementById("root"));
  };
  ...
  if (Object.prototype.toString.call(model) === "[object Array]") {
    ...
    model.forEach(val => {
      tempModels = Object.assign(
        tempModels,
        modelReducersToStore(val, persist)
      );
    });
...
function checkNameSpace(action) {
  if (
    action.type.toString().indexOf("/") === -1 &&
    action.type.toString().indexOf("@") === -1
  ) {
