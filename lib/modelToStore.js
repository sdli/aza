...
function createApp(options, model) {
    ...
    var __reducers = addRoutingModel(modelReducersToStore(model, options.persist));
    var store = (0, _redux.createStore)(

    // 从model中，提取reducers
    // 此处判断是否为immutable，如果是，则使用ImmutableCombineReducers
    options || options && options.immutable || options && typeof options.immutable === "undefined" ? (0, _reduxImmutable.combineReducers)(__reducers) : (0, _redux.combineReducers)(__reducers), options.initialState ? _immutable2.default.fromJS(Object.assign(options.initialState, getReduxPersis(options.persist))) : _immutable2.default.fromJS(getReduxPersis(options.persist)),
    /**
